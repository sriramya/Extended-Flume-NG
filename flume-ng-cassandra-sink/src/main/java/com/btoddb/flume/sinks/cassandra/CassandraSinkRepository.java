...
// Add call for get keyspace
            ...
            ByteBuffer recordsKey = ByteBuffer.wrap(new String(src + ":" + key).getBytes());// row key which have to be unic

            job.beginWorkUnit();
            Mutator<ByteBuffer> m = job.getMutator();
            m.addInsertion(recordsKey, recordsColFamName, HFactory.createStringColumn("timestamp", tsAsStr));
            m.addInsertion(recordsKey, recordsColFamName, HFactory.createStringColumn("source", src));
            m.addInsertion(recordsKey, recordsColFamName, HFactory.createStringColumn("host", host));
            m.addInsertion(recordsKey, recordsColFamName, HFactory.createStringColumn("data", event.getBody().toString()));
            job.submitWorkUnit();
        }
